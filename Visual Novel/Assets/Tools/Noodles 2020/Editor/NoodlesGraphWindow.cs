...
namespace nullbloq.Noodles
{
	...
	public class NoodlesGraphWindow : EditorWindow
	{
		...
		{
			var window = GetWindow<NoodlesGraphWindow>();
			window.titleContent = new GUIContent("Noodles");
		}
		...
		private static void CreateNewNoodle()
		{
			...
			startNode.Set(new Vector2(300,200), Guid.NewGuid().ToString());
			...
			endNode.Set(new Vector2(500, 200), Guid.NewGuid().ToString());
